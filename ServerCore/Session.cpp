...
void Session::ProcessDisconnect()
{
	...
	// BroadCasting 중에 Disconnect되면 Crash 나는 오류 수정
...
int32 PacketSession::OnRecv(BYTE* _buffer, int32 _len)
{
	...
	while (true)
	{
		...
		// 최소한 PacketHeader가 파싱이 가능한가?
		if (dataSize < sizeof(PacketHeader))
			break;

		PacketHeader header = *(reinterpret_cast<PacketHeader*>(&_buffer[0]));
		// PacketHeader에 기록된 패킷 크기를 파싱이 가능한가?
		if (dataSize < header.size)
			break;

		// 패킷 조립 성공
		OnRecvPacket(&_buffer[0], header.size);
